...
typedef struct VVCInterDSPContext {
    ...
    // @TODO: Make this an array where one element is (a shim to) the standard emulated_edge_mc.  This can then be used in the case src_step_x = src_step_y = 0, s.t. assembly optimisations are still used for typical blocks.
